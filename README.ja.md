...
> 役割は、アカウントではありません。3つとも自分一人で規律として回してもいいし、別々の identity（別 GitHub アカウント＋branch protection）に割り当てて強制してもいい。まず役割から始め、必要になったら強制に上げる（[`docs/ja/identity-hardening.md`](docs/ja/identity-hardening.md)）。
...
- 🟢 **低**：変更5ファイル以下・削除なし・保護対象に触れない・低リスクのラベル → レビュー後、実装役がそのまま merge
- 🟡 **中**：20ファイル以下・削除3件以下・保護対象に触れない → レビュー → 通知 → 一拍置いて merge
- 🔴 **高**：20ファイル超 ／ 削除4件以上 ／ 差分1000行超・保護対象（CI／秘密情報／依存／マイグレーション／インフラ／不可逆）・バイナリ・重大ラベル → **人間の承認が必須。自動 merge 禁止**
...
速いAIレビューには、これも全部「OK」の緑チェックに見えます。Qorum の答えは「もっと厳しくレビューしろ」ではなく、**リスクで仕分けて、危ないものだけは人間で止める**こと。レビュー役のAIは、そこを通せないようにしておきます。
...
3. （動き出したら）規約だけでなくプラットフォームでも強制する：1承認必須・stale 承認の自動失効・force push 禁止（[`docs/ja/identity-hardening.md`](docs/ja/identity-hardening.md)）

`.qorum.yml` と `AGENTS.md` を英語のままにしているのは、コピーして使う実用品なので汎用性を優先したためです。

## ツール / 連携

- **`tools/qorum_classify.py`** — 機械的なゲート。`git diff` ＋ `.qorum.yml` から変更の tier を判定（[`tools/README.md`](tools/README.md)・英語）。コピーした `.qorum.yml` に、ついに消費者ができる
- **`skill/qorum/`** — Claude スキル。コードだけでなく、戦略・採用・草案などの意思決定にも同じ階層を当てる（検証の中心はコードゲート）。差分が無い判断のための形

## 合成リスク — 1変更ずつの分類が見落とすもの

Qorum は1変更ずつ仕分ける。その盲点は、**リスクは積み重なる**ことです。個別には Low の変更でも、同じ秘密情報・顧客・リリース・予算に作用するものが重なれば、合算で Medium や High になる。diff だけからは、それが見えません。

ゲートの上に、層を置く。いま走っている全部を、後戻りしにくい面に対して定期で見渡す——集約レイヤー（portfolio review）。個別には問題ない変更が、ひとつの不可逆なもの（ひとつの秘密情報・顧客・ブランド・ローンチ・資金）に収束するなら、各々の tier に関わらず集約レベルで引き上げる。
...
「思想」だけでは信用できません。このリポジトリ自身を、自分のレビュー運用に通したら、AIレビューが自分の穴（どの段にも当てはまらない＝分類の抜け穴）を見つけ、直しています。

→ 具体的に何を止めるか、と、自己点検の実録は [`docs/ja/cases.md`](docs/ja/cases.md)

## なぜこの形なのか

ホワイトボードで設計したのではなく、失敗のあとに残ったものです（[`docs/ja/rationale.md`](docs/ja/rationale.md)）。AIに「信用できる相手であれ」と求めるのをやめ、**通してよい範囲を縛れば、信用の問題そのものが消える**——これが核心です。
